...
%   folder. Blocks (each toggleable):
%     (1) CV_L sweep at a representative mean -> speed & energy vs CV_L.
%     (1b) precision sweep -> timing jitter vs CV_L.
...
%     'NoiseAmp_nA'    noise intensity -- CALIBRATE with test_step2 first (default 0.02)
%     'BaseSeed'       sampler/geometry seed base (default 7000)
%     'DoCVEnergy'     run the deterministic speed/energy block 1 (default true).
%                      Block 2 (trade-off) needs block 1, so it is skipped too
%                      when this is false.
%     'DoPrecision'    run the precision sweep block 1b (default true)
...
doCVE    = getOpt(varargin, 'DoCVEnergy',     true);
...
if doCVE
    ...
    fprintf('\n[1] CV/energy block skipped (DoCVEnergy=false)\n');
end
...
if doCVE && ~isempty(Rc)
    ...
    fprintf('[2] Trade-off figure skipped (needs the CV/energy block)\n');
end
