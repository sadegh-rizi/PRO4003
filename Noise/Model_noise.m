function [MEMBRANE_POTENTIAL, INTERNODE_LENGTH, TIME_VECTOR] = Model_noise(par, filename, isVerbose)
...
% ======================= NOISE SETUP ================================ %
% Gaussian current noise added to nodal intracellular RHS current.
% par.noise.amp.value is interpreted as the SD of the current sample.
% Units should usually be nA: par.noise.amp.units = {1, 'nA', 1}
%
% The model's internal current unit is uA, so simunits converts nA -> uA.

noiseStd = 0;
noiseSegmentIdx = nodes;        % add noise only to node segments
% noiseSegmentIdx(1:nns) = [];  % exclude stimulation node noise

if ~isempty(par.noise.seed)
    rng(par.noise.seed);
end

noiseStd = simunits(par.noise.amp.units) * par.noise.amp.value;

% if isfield(par, 'noise') && isfield(par.noise, 'amp')
%     if isfield(par.noise.amp, 'value') && ~isempty(par.noise.amp.value)
%         noiseStd = simunits(par.noise.amp.units) * par.noise.amp.value;
%     end
% end
% 
% % Optional: provide seed else random seed
% if isfield(par, 'noise') && isfield(par.noise, 'seed')
%     if ~isempty(par.noise.seed)
%         rng(par.noise.seed);
%     end
% end
% 
% % Optional: exclude stimulated first node else all nodes have noise
% if isfield(par, 'noise') && isfield(par.noise, 'excludeStimNode')
%     if par.noise.excludeStimNode
%         noiseSegmentIdx(1:nns) = [];
%     end
% end
...
for i = 1 : T
    ...
    % Add Gaussian noise to segment intracellular compartment.
    V1(noiseSegmentIdx, 1) = V1(noiseSegmentIdx, 1) ...
        + noiseStd * randn(numel(noiseSegmentIdx), 1);
