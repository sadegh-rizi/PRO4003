...
%       .sigma_per_mm (ms/mm), .nValid, .failed_rate, .all_arrivals_ms, plus .table.
%
...
% Preallocate space to track raw latency value for each individual trial
all_arrivals = nan(nL, nTrials); 
...
for q = 1:nL
    ...
    try
        ...
        % Store full array of raw metrics for this specific length index
        all_arrivals(q, :) = arrivals; 
    ...
    % Crash-safe checkpoint saves after each length configuration resolves.
    if ~isempty(saveCsv)
        writePrecisionCsv(saveCsv, Lvalues_um, L_act, nIntn, nNode, dNode, dist, ...
                          meanLat, sig, signorm, nValid, failed_rate);
                      
        % Generate distinct companion path structure for long-format single trials
        [pathstr, name, ext] = fileparts(saveCsv);
        raw_csv_name = fullfile(pathstr, [name '_raw_trials' ext]);
        writeRawTrialsCsv(raw_csv_name, Lvalues_um, all_arrivals);
    end
end
...
R.all_arrivals_ms = all_arrivals; 
...
function writeRawTrialsCsv(fname, L_values, all_arrivals)
% Writes long-format independent trial entries for seamless Python pandas parsing
fid = fopen(fname, 'w');
if fid < 0, return, end
fprintf(fid, 'L_um,Trial_Index,ArrivalTime_ms\n');
[nL, nTrials] = size(all_arrivals);

for i = 1:nL
    for t = 1:nTrials
        % Prevents writing empty rows for values that haven't been simulated yet
        if ~isnan(all_arrivals(i, t)) || i == 1
            fprintf(fid, '%g,%d,%g\n', L_values(i), t, all_arrivals(i, t));
        end
    end
end
fclose(fid);
