...
for il = 1 : n_lengths
    ...
    for trial =  1: n_trials
        % run the model
        [MEMBRANE_POTENTIAL, ~ , TIME_VECTOR] = Model(par_modified); 
        ...
        delays(trial,1) = conduction_delay;
    end
    ...
    mean_delay(il) = mean(delays(valid));
    std_delay(il) = std(delays(valid));
    norm_jitter(il) = std_delay(il) / sqrt(L_total_um); 
